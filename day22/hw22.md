...
greedy, not recursive, and not dynamic programming.

b. If the text contains a word that is longer than `max_width`, what does this algorithm do?
It puts the whole word on its own line and adds it to lines, without checking to see if the word is too long. This means the algorithm could exceed the set line length.

c. Does this algorithm produce an optimal result? (Do you have enough information to answer this question?)
I would say that granted its inability to properly hold to a line length, any result would fail to be optimal because it doesn't do what its designed to do.
...
a. Is this algorithm any of: greedy, recursive, dynamic programming (top down), dynamic programming (bottom up)?
This seems to be a mixture of greedy and bottom up dynamic programming. Beginning with the bottom up, thend deciding at each step what the local best is.
